...
> 版本：V2.1（缴费服务 + Local-First 本地优先开发版）  
...
禁止在 Page、Vue View、业务 Service 中通过硬编码判断环境：
...
## 1.1.2 Development 本地运行拓扑

若仓库已有成熟技术栈，优先沿用，不要为了本规则强制重构。
...
维修师傅移动端
    优先与业主小程序复用同一小程序工程 / 公共组件与 Service
    通过角色与路由权限隔离
...
开发阶段不要求所有服务都运行在 Docker 中。目标是：

```text
修改小程序代码 → 立即重新编译
修改 Vue → Vite 热更新
修改 Python → uvicorn 自动 reload
数据库 → 使用与生产尽量一致的 MySQL 行为
```
...
所有表结构变更必须通过迁移工具管理；若采用 SQLAlchemy，则使用 Alembic。
