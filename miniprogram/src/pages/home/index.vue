...
<template>
  <view class="phone-page">
    <view class="hero">
      <view class="topline"><view class="house">{{ store.session?.house?.displayName || '正在加载房屋...' }} <text>⌄</text></view><view class="mode">本地开发</view></view>
    ...
    <view class="content">
      ...
      <view class="section-title"><text>账单提醒</text><text @click="uni.navigateTo({url:'/pages/payment/list'})">查看全部 ›</text></view>
      <view v-if="bills.find(item => ['PENDING','OVERDUE'].includes(item.status))" class="bill-reminder card" @click="uni.navigateTo({url:'/pages/payment/list'})">
      ...
      <view class="section-title"><text>最新公告</text><text @click="uni.switchTab({url:'/pages/notice/index'})">查看更多 ›</text></view>
