...
async function pay(result='success'){
  paying.value=true
  try{const response=await api.pay(id,result);uni.showToast({title:response.status==='SUCCESS'?'缴费成功':response.status==='CLOSED'?'已取消支付':'支付失败',icon:response.status==='SUCCESS'?'success':'none'});await load()}finally{paying.value=false}
}
function choosePay(){uni.showActionSheet({itemList:['模拟支付成功','模拟用户取消','模拟支付失败'],success:({tapIndex})=>pay(['success','cancel','fail'][tapIndex])})}
...
<template>
  <view v-if="bill" class="page detail-page">
    ...
    <view class="bottom safe-bottom"><view><text>合计金额：</text><strong>¥{{ bill.totalAmount }}</strong></view><button v-if="bill.status!=='PAID'" class="primary-btn" :loading="paying" @click="choosePay">立即缴费</button><button v-else class="primary-btn" @click="applyReceipt">申请收据</button></view>
  </view>
</template>
