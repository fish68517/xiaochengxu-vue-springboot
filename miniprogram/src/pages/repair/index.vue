...
async function submit(){if(description.value.trim().length<5)return uni.showToast({title:'请至少填写5个字的故障描述',icon:'none'});submitting.value=true;try{await api.createRepair({description:`${type.value}：${description.value}`,contactName:store.session?.displayName||'',contactPhone:store.session?.phone||'',imageUrls:[]});uni.showToast({title:'报修已提交',icon:'success'});description.value='';active.value='list';await load()}finally{submitting.value=false}}
...
<template>
  <view class="page repair-page">
    ...
    <template v-if="active==='create'">
      ...
      <view class="upload-tip card"><view>＋</view><text>上传故障图片</text><small>Development 本地存储 Adapter 已启用</small></view><button class="primary-btn submit" :loading="submitting" @click="submit">提交普通故障报修</button>
    </template>
