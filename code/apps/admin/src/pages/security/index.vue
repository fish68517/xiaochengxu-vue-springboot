<template>
  <AdminShell title="安全中心">
    <view class="security-page">
      <view v-if="required" class="required-banner">
        <view class="banner-mark">!</view>
        <view><text>必须先修改密码</text><text>当前账号仍在使用初始密码或弱密码，修改完成并重新登录后才能继续使用管理功能。</text></view>
      </view>

      <view class="security-grid">
        <view class="panel">
          <text class="panel-title">修改登录密码</text>
          <text class="panel-desc">密码修改后，当前会话会立即失效，需要使用新密码重新登录。</text>
        ...
        <view class="tips-panel"><text>安全建议</text><text>不要使用手机号、账号名或连续数字作为密码。</text><text>不要与其他网站共用同一密码。</text><text>平台工作人员不会索要您的密码或验证码。</text></view>
      </view>
    </view>
  </AdminShell>
</template>
...
function clearSession() {
  for (const key of ['token', 'user', 'accessProfile', 'mustChangePwd', '__admin_active_brand__']) uni.removeStorageSync(key);
}
