// 环境化请求层(T0-08):统一 action 协议,按平台选 transport —— 小程序=uniCloud.callFunction,H5=VITE_API_BASE HTTP。
...
function callCloud(action, payload, token) {
    ...
    uniCloud.callFunction({
      ...
      success: (res) => {
        ...
        if (r && typeof r === 'object' && r.ok === false) {
          reject(new ApiError(r.message || '业务处理失败', { code: r.code, status: r.statusCode }));
        } else {
          resolve(r);
        }
      },
      // 云函数抛错/基础设施失败走 fail:按可重试网络错处理(业务错契约应为 result.ok===false)。
      fail: (err) => reject(new ApiError((err && err.errMsg) || '云函数调用失败', { isNetwork: true })),
    });
  });
}
...
async function doCall(action, payload = {}, { write = false } = {}) {
  ...
  const useCloud = !API_BASE && cloudAvailable();
  if (!API_BASE && !useCloud) {
    throw new ApiError('未配置后端地址(VITE_API_BASE)', { isNetwork: true });
  }
