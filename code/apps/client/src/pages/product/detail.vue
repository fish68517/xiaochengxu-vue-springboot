...
async function orderNow(){if(!product.value)return;ordering.value=true;try{const res=await getH5OrderToken(product.value.id||product.value._id);if(!res||!res.token)throw new Error('下单链接生成失败');uni.navigateTo({url:`/pages/order/webview-shell?token=${encodeURIComponent(res.token)}`});}catch(error){uni.showToast({title:(error&&error.message)||'下单失败',icon:'none'});}finally{ordering.value=false;}}
