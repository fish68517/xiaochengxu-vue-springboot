...
<script setup>
import { ref } from 'vue'; import { onLoad,onShow } from '@dcloudio/uni-app'; import { api,ensureSession } from '../../api.js'; import { brandState } from '../../brand.js'; import { brandContact } from '../../brand-assets.js'; import { fenToYuan,coverOf,gameText,serviceTypeText } from '../../client-utils.js'; import UiSkeleton from '../../components/UiSkeleton.vue'; import UiState from '../../components/UiState.vue';
...
async function orderNow(){if(!product.value)return;ordering.value=true;try{await ensureSession();const res=await api.h5Token(product.value.id||product.value._id);if(!res||!res.token)throw new Error('下单链接生成失败');uni.navigateTo({url:`/pages/order/webview-shell?token=${encodeURIComponent(res.token)}`});}catch(error){uni.showToast({title:(error&&error.message)||'下单失败',icon:'none'});}finally{ordering.value=false;}}
function showH5Contact(){
  // #ifdef H5
  const contact=brandContact();const lines=[contact.serviceWechat&&`客服微信：${contact.serviceWechat}`,contact.servicePhone&&`客服电话：${contact.servicePhone}`,contact.serviceHours&&`服务时间：${contact.serviceHours}`].filter(Boolean);uni.showModal({title:'联系客服',content:lines.join('\n')||'请联系品牌客服获取帮助',showCancel:false});
  // #endif
}
</script>
