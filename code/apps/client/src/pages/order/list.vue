...
const tabs=[{key:'ALL',label:'全部'},{key:'PENDING_PAYMENT',label:'待支付'},{key:'PENDING_ACCEPT',label:'待受理'},{key:'PENDING_GRAB',label:'待抢单'},{key:'ASSIGN_PENDING',label:'待指派确认'},{key:'IN_SERVICE',label:'服务中'},{key:'PENDING_CONFIRM',label:'待验收'},{key:'SETTLED',label:'已结单'},{key:'REFUNDING',label:'售后'}];
const filteredOrders=computed(()=>{if(activeTab.value==='ALL')return orders.value;if(activeTab.value==='REFUNDING')return orders.value.filter((item)=>['REFUNDING','REFUNDED','DISPUTING'].includes(item.status));return orders.value.filter((item)=>item.status===activeTab.value);});
onLoad((query)=>{if(query&&query.status&&tabs.some((tab)=>tab.key===query.status))activeTab.value=query.status;});
onShow(()=>{try{uni.setNavigationBarTitle({title:(brandState.brand&&brandState.brand.name)||'我的订单'});}catch(e){/* 忽略 */}loadOrders();});
