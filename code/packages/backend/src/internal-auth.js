...
export function signInternalRequest({ action, payload = {}, secret, timestamp = Date.now(), nonce = randomBytes(18).toString('base64url') }) {
...
export function verifyInternalRequest({ action, payload = {}, auth, secret, now = Date.now(), maxSkewMs = INTERNAL_AUTH_MAX_SKEW_MS }) {
  ...
  let actual;
  try { actual = Buffer.from(String(auth.signature), 'base64url'); } catch { throw new Error('内部请求签名非法'); }
